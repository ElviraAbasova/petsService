...
const Basket = () => {
  ...
  const handleCount=()=>{
    return basket.reduce((acc, elem) => acc + elem.count, 0);
  };

  const handleSum = () => {
    return Math.round(basket.reduce((acc, elem) => acc + (elem.count * (elem.price*(100-elem.discount)/100)), 0) * 100) / 100;
  };
  ...
  return (
    <section id="basket">
      ...
      <ToastContainer/>
      <Checkout isOpen={isModalOpen} onClose={closeModal} sum={handleSum()+5} /> 
    </section>
  );
};
